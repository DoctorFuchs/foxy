...
class PatternDoesntMatch(BaseException): pass
class UnsupportedFileType(BaseException): pass
class tableNotFound(BaseException): pass
...
class table:
    ...
    def getCells(self, pos_):
        if not pos_.upper():
            ...
            raise PatternDoesntMatch(config["ERRORS"]["patternExampleMultipleCells"])

        #TODO getCells like in excel
        raise FeatureInDevelopment("At the moment this functions is unvailable.")
    ...
    def getCell(self, pos_: str) -> cell:
        if not pos_.upper():
            ...
            raise PatternDoesntMatch(config["ERRORS"]["patternExampleSingleCell"])
...
class database:
    ...
    def createTable(self, name = None, parser_=defaultParser):
        if name is None:
            ...
            if name in self.tables: return self.getTable(name) 
            else: return table(master_ = self, name_= name, parser_=parser_)
