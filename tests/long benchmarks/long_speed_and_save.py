...
def check() -> Union[bool, bool]:
    ...
    print("DATABASE CREATE TASK FINISHED (after {} since start)".format(str(datetime.now()-time_start)), file=sys.stderr)
    ...
    print("TABLE CREATE TASK FINISHED (after {} since start)".format(str(datetime.now()-time_start)), file=sys.stderr)
    d.save()

    print("DATABASE SAVE TASK FINISHED (after {} since start)".format(str(datetime.now()-time_start)), file=sys.stderr)
    ...
    print("TABLE CREATE TASK FINISHED (after {} since start)".format(str(datetime.now()-time_start)), file=sys.stderr)
    cursor.fetchall()
    print("DATABASE FETCH TASK FINISHED (after {} since start)".format(str(datetime.now()-time_start)), file=sys.stderr)
    ...
    print("SUCCESSFUL" if data2.tables.keys() == d.tables.keys() else "FAILED", file=sys.stderr)
